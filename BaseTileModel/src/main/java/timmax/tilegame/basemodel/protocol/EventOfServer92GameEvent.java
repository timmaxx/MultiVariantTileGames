...
public class EventOfServer92GameEvent extends EventOfServer {
    ...
    public void executeOnClient(LocalClientStateAutomaton localClientStateAutomaton) {
        ...
        // 3. клиент, получив событие типа 71SetGameMatchIsPlaying (2.1.) должен построить одну или несколько выборок,
        //    в которых и будут потом отображаться игровые события.
        // В этом коде 'localClientStateAutomaton.getView(viewName)' может вернуть null. Так может получиться, если
        // сервер успевает отправить клиенту событие типа 92GameEvent (п. 2.2.) (и клиент его принимает),
        // но клиент ещё не успел создать выборку, для которой поступило событие (т.е. не выполнил п. 3).
        // Поэтому сейчас, если 'localClientStateAutomaton.getView(viewName == null', то немного ждём в этом потоке,
        // пока другой поток (обрабатывающий предыдущее событие), не создаст выборки.
        try {
            View view;
